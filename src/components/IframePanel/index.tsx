...
import { dslToHtml } from './dslToHtml'

export default defineComponent({
  name: 'IframePanel',
  props: {
    dslNodes: { type: Array as PropType<DslNode[]>, default: () => [] },
  },
  setup(props) {
    const inputUrl  = ref('')
    const iframeSrc = ref('')        // reactive src bound directly to iframe
    const loading   = ref(false)
    const isDsl     = ref(false)
    let   blobUrl   = ''

    function revokePrevBlob() {
    ...
    function loadDsl() {
      revokePrevBlob()
      const html  = dslToHtml(props.dslNodes)
    ...
    const EmptyState = () => (
      <div class="flex-1 flex flex-col items-center justify-center gap-4 text-gray-400 select-none">
        ...
        <div class="text-center">
          <p class="text-sm font-medium">输入地址或加载 DSL 预览</p>
          <p class="text-xs mt-1 opacity-60">在顶部地址栏输入 URL，或点击「DSL 预览」</p>
        </div>
      </div>
    )
      ...
      <div class="flex flex-col h-full bg-white">
        ...
        <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-100 flex-shrink-0">
          ...
          <div class="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-lg px-3 h-8 gap-2 focus-within:border-blue-300 focus-within:ring-1 focus-within:ring-blue-200 transition-all">
            {isDsl.value && (
              <span class="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded font-medium flex-shrink-0">DSL</span>
            )}
            <input
              ...
              onInput={(e: Event) => { inputUrl.value = (e.target as HTMLInputElement).value; isDsl.value = false }}
          ...
          <button
            class={[
              'px-3 h-8 rounded-lg text-xs font-medium transition-colors flex-shrink-0 border',
              isDsl.value
